...
/**
 * @file
 * Contains \Drupal\bootstrap_lite\BootstrapLite.
 */

namespace Drupal\bootstrap_lite;

use Drupal\bootstrap_lite\Plugin\AlterManager;
use Drupal\bootstrap_lite\Plugin\FormManager;
...
/**
 * The primary class for the Drupal Bootstrap Lite base theme.
 *
...
class BootstrapLite {
   ...
   * @var string
   */
  const PROJECT_PAGE = 'https://www.drupal.org/project/bootstrap_lite';
   ...
   */
  public static function addCallback(array &$callbacks, $callback, $replace = NULL, $action = BootstrapLite::CALLBACK_APPEND) {
    // Replace a callback.
    if ($replace) {
  ...
  public static function alter($function, &$data, &$context1 = NULL, &$context2 = NULL) {
    ...
    if (strpos($hook, 'form') === 0 && $context1 instanceof FormStateInterface) {
      ...
      foreach ($ids as $id) {
        /** @var \Drupal\bootstrap_lite\Plugin\Form\FormInterface $form */
    ...
    // Process hook alter normally.
      ...
      /** @var \Drupal\bootstrap_lite\Plugin\Alter\AlterInterface $class */
  ...
  public static function apiSearchUrl($query = '') {
    return self::PROJECT_DOCUMENTATION . '/api/bootstrap_lite/' . self::PROJECT_BRANCH . '/search/' . Html::escape($query);
  }
  ...
  public static function autoloadFixInclude() {
    return static::getTheme('bootstrap_lite')->getPath() . '/autoload-fix.php';
  }

  /**
   * Matches a Bootstrap Lite class based on a string value.
   *
   ...
   *
   * @return string
   *   The Bootstrap Lite class matched against the value of $haystack or $default
  ...
  public static function cssClassFromString(&$value, $default = '') {
    ...
    if ($texts->isEmpty()) {
      ...
      $theme_manager->alter('bootstrap_lite_colorize_text', $data);
  ...
  /**
   * Retrieves a theme instance of \Drupal\bootstrap_lite.
   *
   * @param string $name
   ...
   * @see bootstrap_lite_theme_registry_alter()
   * @see bootstrap_lite_theme()
   * @see hook_theme()
   */
  public static function getThemeHooks() {
    $hooks['bootstrap_lite_carousel'] = [
      'variables' => [
        'attributes' => [],
        'controls' => TRUE,
        'id' => NULL,
  ...
  public static function glyphicon($name, $default = []) {
    ...
    // @todo Supply a specific version to _bootstrap_lite_glyphicons() when Icon API
    // supports versioning.
  ...
  public static function glyphiconFromString(&$value, $default = []) {
    ...
    if ($texts->isEmpty()) {
      ...
      /** @var \Drupal\Core\Theme\ThemeManager $theme_manager */
      $theme_manager = \Drupal::service('theme.manager');
      $theme_manager->alter('bootstrap_lite_iconize_text', $data);
   ...
   *   TRUE or FALSE
   *
   * @see \Drupal\bootstrap_lite\BootstrapLite::isFront
   ...
   *   TRUE or FALSE
   *
   * @see \Drupal\bootstrap_lite\BootstrapLite::hasIsFrontCacheContext
   * @see \Drupal\bootstrap_lite\BootstrapLite::preprocess
   * @see https://www.drupal.org/node/2829588
   */
  ...
  public static function preprocess(array &$variables, $hook, array $info) {
    ...
    if (isset($info['bootstrap_lite preprocess'])) {
      if ($preprocess_manager->hasDefinition($info['bootstrap_lite preprocess'])) {
        $class = $preprocess_manager->createInstance($info['bootstrap_lite preprocess'], ['theme' => $theme]);
        /** @var \Drupal\bootstrap_lite\Plugin\Preprocess\PreprocessInterface $class */
