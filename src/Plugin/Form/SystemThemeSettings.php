...
/**
 * @file
 * Contains \Drupal\bootstrap_lite\Plugin\Form\SystemThemeSettings.
 */
...
class SystemThemeSettings extends FormBase implements FormInterface {
  ...
  public function alterFormElement(Element $form, FormStateInterface $form_state, $form_id = NULL) {
    ...
    // Creates the necessary groups (vertical tabs) for a Bootstrap Lite based theme.
  ...
  protected function createGroups(Element $form, FormStateInterface $form_state) {
    ...
    $form['bootstrap_lite'] = [
      '#type' => 'vertical_tabs',
      '#attached' => ['library' => ['bootstrap_lite/theme-settings']],
      '#prefix' => '<h2><small>' . t('Bootstrap Lite Settings') . '</small></h2>',
      '#weight' => -10,
    ];
    $groups = [
      'general' => t('General'),
   ...
   *   The current state of the form.
   *
   * @return \Drupal\bootstrap_lite\Theme|FALSE
   *   The currently selected theme object or FALSE if not a Bootstrap Lite theme.
   */
  public static function getTheme(Element $form, FormStateInterface $form_state) {
    $build_info = $form_state->getBuildInfo();
    $theme = isset($build_info['args'][0]) ? BootstrapLite::getTheme($build_info['args'][0]) : FALSE;

    // Do not continue if the theme is not Bootstrap Lite specific.
